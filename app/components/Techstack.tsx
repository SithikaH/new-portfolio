...
export default function TechStack() {
  // ✅ FIXED TYPE
  const stackWordRef = useRef<HTMLSpanElement>(null);

  // 📝 GSAP FALLING EFFECT
  useEffect(() => {
    if (!stackWordRef.current) return;
      ...
      <div className="max-w-6xl mx-auto">
          ...
          <h2 className="text-5xl md:text-7xl font-black text-white uppercase tracking-tighter flex flex-wrap gap-x-4">
            ...
            <span ref={stackWordRef} className="flex">
              {"MANIFEST".split("").map((char, i) => (
                <span
                  key={i}
                  className="letter inline-block text-transparent"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-white/10 border border-white/10 mb-px">
          {mainTech.map((tech, i) => (
            <div
              key={i}
              className="bg-[#0a0a0a] p-10 group hover:bg-[#111] transition-all duration-500 relative overflow-hidden"
            >
              ...
              <div className="relative z-10 space-y-8">
                <div className="flex justify-between items-start">
                  <tech.Icon
                    size={48}
                    className="text-white/40 group-hover:text-[#36ecde] transition-all duration-500 group-hover:scale-110"
                  />
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="h-[2px] w-full bg-white/5">
                      <motion.div 
                        ...
                        className="h-full bg-[#36ecde] shadow-[0_0_10px_#36ecde]"
                      />
                    </div>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-px bg-white/10 border-x border-b border-white/10">
          {subTech.map((tech, i) => (
            <div
              key={i}
              className="bg-[#0a0a0a] py-12 flex flex-col items-center justify-center gap-4 group hover:bg-[#111] transition-colors"
            >
              <tech.Icon
                size={32}
