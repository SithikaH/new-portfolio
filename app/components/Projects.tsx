    ...
    <section id="projects" className="bg-[#050505] text-white py-32 px-6 md:px-16 border-t border-white/5">
      <div className="max-w-7xl mx-auto">
        ...
        <div ref={gridRef} className="relative">
          ...
          <div className="absolute left-1/3 top-0 w-[1px] hidden lg:block h-full bg-white/5 overflow-hidden z-10">
             <div className="line-v w-full bg-[#36ecde]/40 absolute bottom-0" />
          </div>
          <div className="absolute left-2/3 top-0 w-[1px] hidden lg:block h-full bg-white/5 overflow-hidden z-10">
             <div className="line-v w-full bg-[#36ecde]/40 absolute bottom-0" />
          </div>

          {/* Horizontal Grid Lines */}
          <div className="absolute top-0 left-0 w-full h-[1px] bg-white/5 overflow-hidden z-10">
            <div className="line-h h-full bg-[#36ecde]/40 absolute left-0" />
          </div>
          <div className="absolute bottom-0 left-0 w-full h-[1px] bg-white/5 overflow-hidden z-10">
            <div className="line-h h-full bg-[#36ecde]/40 absolute right-0" />
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-white/10 border border-white/10 overflow-hidden">
            {/* SAFETY CHECK: Use the array variable correctly */}
      ...
      <div className="p-8 flex flex-col justify-between flex-grow">
        <div>
          <div className="flex gap-2 mb-4">
            {tags.map((tag, i) => (
              <span key={i} className="text-[9px] font-mono text-[#36ecde] border border-[#36ecde]/30 px-2 py-0.5 rounded uppercase">
                {tag}
              </span>
            ))}
          </div>
          <h3 className="text-2xl font-black uppercase tracking-tight mb-4 group-hover:text-[#36ecde] transition-colors">
            {title}
          </h3>
        ...
        <div className="flex gap-6 mt-auto">
          {github && (
            <a href={github} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2 text-[11px] font-mono uppercase tracking-widest text-white/40 hover:text-white transition"
            >
              <FiGithub size={16} className="text-[#36ecde]" /> Source
            </a>
          )}
          {live && (
            <a href={live} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2 text-[11px] font-mono uppercase tracking-widest text-white/40 hover:text-white transition"
            >
              <FiExternalLink size={16} className="text-[#36ecde]" /> Deployment
            </a>
          )}
        </div>
      </div>
